import { SETS_TABLE } from '../constants/table.constants';
import { auth, validate, Router, jsonBodyParser } from '../middlewares';
import { CRUDService, SerializeService, QueryService } from '../services';
...
setsRouter
  ...
  .get(async (req, res, next) => {
    try {
      const emptySets: Express.SetList[] = await CRUDService.getAllData(
        req.app.get('db'),
        SETS_TABLE,
        res.user.id
      );
